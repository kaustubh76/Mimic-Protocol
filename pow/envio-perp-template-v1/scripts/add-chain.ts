#!/usr/bin/env node
/**
 * add-chain — multi-chain expansion CLI for the perp template.
 *
 * Implements ENVIO_MULTICHAIN_EXPANSION_RUNBOOK.md as a deterministic,
 * idempotent CLI. Same shape as the PM template's add-chain.ts; the
 * Constants.ts type matches the perp template (no factoryAddress in the
 * type — that lives only in config.yaml).
 *
 * USAGE
