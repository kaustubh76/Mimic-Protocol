...
export class PatternValidator {
    // Risk thresholds
    static MIN_SAFE_WIN_RATE = 55; // 55%
    static MAX_CONSECUTIVE_LOSSES = 5;
    static MAX_DRAWDOWN_PERCENT = 30; // 30%
    static MIN_TRADE_COUNT = 10; // Minimum trades for reliable scoring
    static CONSISTENCY_THRESHOLD = 0.7; // 70% consistency
    static STALE_PATTERN_HOURS = 48; // Pattern not executed in 48 hours
    /**
     * Calculate comprehensive risk score for a pattern
     */
    static calculateRiskScore(metrics, currentTime) {
        const flags = [];
        const reasons = [];
        ...
        const shouldPause = metrics.consecutiveLosses >= this.MAX_CONSECUTIVE_LOSSES ||
     ...
     */
    static calculateConsistency(returns) {
        if (returns.length < 2)
            return 0;
        const avgReturn = returns.reduce((sum, r) => sum + r, 0) / returns.length;
     ...
     */
    static validatePatternHealth(metrics, currentTime) {
     ...
     */
    static shouldTriggerCircuitBreaker(metrics) {
     ...
     */
    static generatePerformanceReport(metrics, currentTime) {
        ...
        if (metrics.winRate >= 70)
            strengths.push('Exceptional win rate');
        else if (metrics.winRate >= this.MIN_SAFE_WIN_RATE)
        ...
        if (metrics.totalTrades >= 50)
            strengths.push('Well-tested pattern');
        if (metrics.consecutiveWins >= 5)
            strengths.push('Strong recent performance');
        // Analyze weaknesses
        if (metrics.winRate < this.MIN_SAFE_WIN_RATE)
            weaknesses.push('Below optimal win rate');
        if (metrics.consistency < this.CONSISTENCY_THRESHOLD)
            weaknesses.push('Inconsistent performance');
        if (metrics.totalTrades < this.MIN_TRADE_COUNT)
            weaknesses.push('Insufficient trading history');
        if (metrics.consecutiveLosses >= 3)
            weaknesses.push('Recent losing streak');
