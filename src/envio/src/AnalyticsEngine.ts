...
import { PatternValidator, type PatternMetrics } from './PatternValidator.js';
...
export interface TrendingPattern {
  ...
  newDelegators24h: number;
  volumeGrowth24h: bigint;
  momentumScore: number; // 0-100
}

export interface PatternComparison {
  pattern1: PatternAnalytics;
  pattern2: PatternAnalytics;
  winner: {
    performance: bigint;
    risk: bigint;
    social: bigint;
...
export class AnalyticsEngine {
   ...
   */
  static async calculatePatternAnalytics(
    tokenId: bigint,
    patternData: any,
    delegations: any[],
    trades: any[]
    ...
    const totalVolumeDelegated = delegations.reduce(
    ...
    const performanceReport = PatternValidator.generatePerformanceReport(
      patternMetrics,
      BigInt(Math.floor(Date.now() / 1000))
    );
    ...
    const popularityScore = this._calculatePopularityScore(
      activeDelegations,
      uniqueDelegators,
      Number(totalVolumeDelegated)
    );
  ...
  static generateLeaderboard(
    ...
    sortBy: 'overall' | 'winRate' | 'volume' | 'delegators' = 'overall',
    limit: number = 10
  ): LeaderboardEntry[] {
    const startTime = Date.now();
    ...
    const leaderboard: LeaderboardEntry[] = scoredPatterns
      .slice(0, limit)
      .map((sp, index) => ({
        rank: index + 1,
   ...
   */
  static identifyTrendingPatterns(
    currentAnalytics: PatternAnalytics[],
    historical24h: PatternAnalytics[]
  ): TrendingPattern[] {
    const startTime = Date.now();
    const trending: TrendingPattern[] = [];
    ...
    for (const current of currentAnalytics) {
      ...
      const momentumScore = this._calculateMomentumScore(
        growthRate,
        delegatorGrowth,
        Number(volumeGrowth)
      );
  ...
  private static _calculateMaxDrawdown(trades: any[]): bigint {
    let peak = 0;
    let maxDrawdown = 0;
    let cumulative = 0;
  ...
  private static _calculateConsecutiveLosses(trades: any[]): number {
    let consecutive = 0;
  ...
  private static _calculateConsecutiveWins(trades: any[]): number {
    let consecutive = 0;
    for (let i = trades.length - 1; i >= 0; i--) {
      if (trades[i].success) {
        consecutive++;
  ...
  private static _calculateVolatility(returns: number[]): number {
  ...
  private static _calculatePopularityScore(
    activeDelegations: number,
    uniqueDelegators: number,
    totalVolume: number
  ): number {
  ...
  private static _calculateUptime(trades: any[], createdAt: number): number {
  ...
  private static _calculateLeaderboardScore(
  ...
  private static _calculateMomentumScore(
    growthRate: number,
  ...
  private static _calculatePerformanceScore(pattern: PatternAnalytics): number {
    return (
      pattern.winRate * 0.4 +
