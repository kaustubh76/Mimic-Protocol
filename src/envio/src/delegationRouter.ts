...
/**
 * Event interfaces (auto-generated by Envio)
 */
...
interface DelegationUpdatedEvent {
  delegationId: bigint;
  percentageAllocation: bigint;
  timestamp: bigint;
  blockNumber: bigint;
  ...
  try {
    ...
    if (!delegatorEntity) {
      ...
      const totalAllocation =
        delegatorEntity.averageAllocation * (delegatorEntity.totalDelegations - 1) +
        Number(percentageAllocation);
      delegatorEntity.averageAllocation = Math.floor(
        totalAllocation / delegatorEntity.totalDelegations
      );
    }
...
export async function handleTradeExecuted(
  ...
  try {
    ...
    if (delegation) {
      ...
      delegation.successRate = Math.floor(
        (delegation.successfulExecutions / delegation.totalExecutions) * 10000
      );
